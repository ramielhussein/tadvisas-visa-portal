...
serve(async (req) => {
  ...
  try {
    ...
    // Simple security - check for a reset token
    if (resetToken !== "RESET_ADMIN_2024") {
    ...
    console.log("Admin user reset successfully:", email);

    return new Response(
      JSON.stringify({ success: true, message: "Admin user reset successfully" }),
      {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      }
